...
Taken from NeHe tutorial 
http://nehe.gamedev.net/data/lessons/lesson.asp?lesson=48

Modified July 2009 to embed more of the logic in the arcball class
...
class ArcBallT:
    ...
    def click(self, NewPt):
        NewPt = Point2fT(*NewPt)
        self.last_rot = copy(self.current_rot)
    ...
    def move(self, NewPt):
        NewPt = Point2fT(*NewPt)
        rot = Matrix3fSetRotationFromQuat4f(self.drag(NewPt))
        self.current_rot = rot = Matrix3fMulMatrix3f(self.last_rot, rot)
        self.transform = Matrix4fSetRotationFromMatrix3f(self.transform, rot)
    
    def reset(self):
        self.transform = Matrix4fT()
        self.last_rot = Matrix3fT()
        self.current_rot = Matrix3fT()
